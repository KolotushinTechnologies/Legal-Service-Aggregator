...
const WithdrawalOfFunds = () => {
  ...
  const replenish = async () => {
    if (text && Number.isInteger(+text.current.value)) {
      await Axios({
        method: 'POST',
        url: 'http://localhost:5000/api/withdrawal-requests',
        data: {
    ...
    <div className='payment__page'>
      {modalStatus && (
        <>
          ...
          <Modal>
            <Row className='title'>
              <div className='modalTitle'>Введите сумму вывода</div>
      ...
      <h1 className='profile__title'>Вывод денежных средств с баланса</h1>
      ...
      <div>
        ...
        {!load ? (
          paymentData[0] ? (
            ...
            <NotReplenishment>
              Заявки на вывод денежных средств с баланса отсутствуют
            </NotReplenishment>
          )
        ) : (
          'Загрузка...'
